...
headless, and log both wrists' force/torque + all 14 joint angles
```
...
- Computes joint targets for each arm: simple per-joint interpolation from the keyframe pose to a pose where the wrist is over the target cube (Drift uses the Aloha forward-kinematics chain to pick reasonable angles — it doesn't claim to be a full IK solver)
