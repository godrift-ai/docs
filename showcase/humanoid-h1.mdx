...
- Writes a `run_sim.py` that:
  ...
  - Applies a per-joint PD controller (stiffer on hips and ankles, lighter on arms — common pattern for hold-pose stability)
  - Steps at 1 ms timestep with `implicitfast` integrator
  - Logs sensors at 200 Hz
- Runs headless, prints the final base height and orientation drift (close to zero if the controller is tuned), and notes any contact issues
...
The H1 stands at the default pose — feet flat on the floor, knees slightly bent, arms by the sides. The IMU log shows the base orientation steady within a few mrad. The torque trace shows the static hold-pose torques on the hips and ankles where most of the gravity load lives.

If you push the robot in the viewer (drag with the right mouse button), it should resist and settle back without falling — that's the controller working.
