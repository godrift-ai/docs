...
No. Drift CLI requires Ubuntu 20.04+ (Debian-based Linux) and x86_64 architecture. macOS and Windows are not supported. ARM-based systems (Raspberry Pi, Apple Silicon) are also not currently supported.

### Does Drift work on ARM (Raspberry Pi, Apple Silicon)?

Not yet. Only x86_64 (amd64) is supported in the current beta. ARM support is planned.
