...
Add one with `/skill add ./team-skills/ekf-tuning.md`. Drift installs it to `~/.drift/skills/<name>/SKILL.md` and uses it automatically from the next prompt onward — add `--repo` to install it into `<repo>/.drift/skills/` instead, so everyone who clones the project gets it. Your team's expertise becomes the agent's.
