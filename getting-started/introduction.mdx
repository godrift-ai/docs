...
Workspace → robot → controllers → world → sim → tuning → docs, one prompt. Drift remembers what it built two steps ago, doesn't hallucinate file paths, and updates URDF + bridge + launch as one coordinated change.
...
### Cost transparency
Per-query token and dollar tracking. See exactly what each robot build cost. Set budgets per workspace in your account dashboard.
